...
import Map, {
  Marker, Source, Layer, NavigationControl, GeolocateControl, Popup, type MapRef, type MapLayerMouseEvent,
} from "react-map-gl/maplibre";
import { 
  LOST_CITIES, ALLIANCES, POPULATION_DENSITY, MIGRATION_FLOWS, FOLKLORE, MUSIC_HERITAGE,
  OCEAN_CURRENTS, SHIPWRECKS, SPACEPORTS, CRATERS, LIGHT_POLLUTION,
  BIOMES, ANCIENT_TREES, ENDANGERED_SPECIES, MEGA_ENGINEERING, NUCLEAR_SITES, RESOURCE_EXTRACTION,
  ANCIENT_WONDERS, MODERN_MARVELS, ABANDONED_PLACES, UNDERSEA_CABLES, CAVE_SYSTEMS, SUBTERRANEAN_TRANSIT,
  WEALTH_CENTERS, ENERGY_GRIDS, GLOBAL_LITERACY, EXTREME_CLIMATES, ANOMALIES, DEEP_SPACE, JOURNEYS
} from "@/lib/paths/data";
...
const HISTORICAL_STYLE = "https://unpkg.com/@openhistoricalmap/map-styles@latest/dist/historical/historical.json";
...
export default function MapView({
  selected, onPick, hover, quakes, showQuakes, flyTo, route, routeEndpoints,
}: Props) {
  const { style, projection, epochYear } = useLayerManager();
  const { 
    trail, showAlliances, showLostCities, showPopulation, showMigration, showFolklore, showMusic, showOceans, showCosmic,
    showBiology, showFootprint, showArchitecture, showUnderground, showEconomics, showExtremes, showJourneys,
    connections, omniscienceLayer, showTraffic, show3D,
    measurementMode, measurementPoints, setMeasurementPoints,
    showTimeline, locationHistory, addLocationHistory,
    contextMenu, setContextMenu, mapViewport, setMapViewport
  } = useUIStore();
  ...
  const { data: pois = [] } = useQuery({
    queryKey: ["pois", poiCategory, bounds?._sw.lat.toFixed(2), bounds?._sw.lng.toFixed(2), bounds?._ne.lat.toFixed(2), bounds?._ne.lng.toFixed(2)],
  ...
  return (
    <div className="relative w-full h-full bg-background">
      {measurementMode && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-panel border border-accent text-accent px-4 py-2 rounded-full font-mono text-[11px] shadow-2xl z-20 flex items-center gap-4 pointer-events-auto">
          ...
          {measurementPoints.length > 0 && (
            <button onClick={() => setMeasurementPoints([])} className="text-muted-foreground hover:text-red-500 uppercase tracking-widest text-[9px]">
              Clear
            </button>
          )}
        </div>
      ...
      <Map
        ref={ref}
        mapStyle={style === "satellite" ? SATELLITE_STYLE : style === "voyager" ? VOYAGER_STYLE : style === "historic" ? HISTORICAL_STYLE : MAP_STYLE}
        ...
        <Marker key={poi.id} longitude={poi.lng} latitude={poi.lat} anchor="bottom">
          <div className="flex flex-col items-center group cursor-pointer">
            <div className="bg-panel border border-line px-2 py-1 rounded shadow-lg flex items-center gap-1.5 transition-transform group-hover:scale-110">
              ...
              <span className="text-[10px] font-mono whitespace-nowrap hidden group-hover:block max-w-[150px] truncate">{poi.name}</span>
            </div>
      ...
      {connections.map((c, i) => (
        <Marker key={`conn-${i}`} longitude={c.lng} latitude={c.lat}>
          <div className="relative group cursor-pointer" onClick={() => useUIStore.getState().handlePick(c)}>
      ...
      {Object.values(useLayerManager().layers).map((layer) => (
        layer.visible && (
          <Source key={`source-${layer.id}`} id={`source-${layer.id}`} type={layer.type as any} data={layer.data} url={layer.url}>
            <Layer
              id={`layer-${layer.id}`}
              type={layer.type === "geojson" && layer.data?.features?.[0]?.geometry?.type === "Point" ? "circle" : layer.type === "geojson" ? "line" : "fill" as any}
