...
export default function NavigationPanel() {
  ...
  const {
    ...
    queryFn: () =>
      getRoute({ lat: from!.lat, lng: from!.lng }, { lat: to!.lat, lng: to!.lng }, mode),
  ...
  const compute = async () => {
    ...
    if (!r) {
      onRoute(null, { from: null, to: null });
      return;
    }
        ...
        >
          ...
          <div className="p-4 space-y-3 border-b border-line">
            ...
            <div className="flex border border-line">
              <ModeBtn active={mode === "driving"} onClick={() => setMode("driving")}>
                <Car className="size-3.5" /> Drive
              </ModeBtn>
              <ModeBtn active={mode === "walking"} onClick={() => setMode("walking")}>
                <PersonStanding className="size-3.5" /> Walk
              </ModeBtn>
              <ModeBtn active={mode === "cycling"} onClick={() => setMode("cycling")}>
                <Bike className="size-3.5" /> Cycle
              </ModeBtn>
            </div>
              ...
              {activeNav ? (
                <div className="p-6 flex flex-col items-center justify-center space-y-6 bg-accent/5 h-[300px]">
                  ...
                  <div className="flex gap-4 w-full mt-4">
                    <button
                      ...
                      className="flex-1 py-3 border border-line text-[11px] tracking-widest uppercase font-mono hover:bg-panel-2 flex items-center justify-center gap-2"
                    >
                      <Square className="size-3.5" /> Stop
                    </button>
                    <button
                      onClick={nextStep}
                      className="flex-1 py-3 bg-accent text-accent-foreground text-[11px] tracking-widest uppercase font-mono hover:opacity-90 flex items-center justify-center gap-2"
                    >
                      Next <Play className="size-3.5" />
                    </button>
                  </div>
                </div>
              ) : (
                <>
                  <div className="p-4 border-b border-line">
                    <button
                      onClick={startNav}
                      className="w-full py-2.5 bg-jade/20 text-jade font-mono uppercase tracking-widest text-[11px] hover:bg-jade/30 flex items-center justify-center gap-2"
                    >
                  ...
                  <ol className="divide-y divide-line-soft">
                    {result.steps.map((s, i) => (
                      <li key={i} className="px-4 py-2 flex gap-3">
                        <span className="font-mono text-[10px] text-accent tabular-nums w-6 mt-0.5">
                          {String(i + 1).padStart(2, "0")}
                        </span>
                        <div className="flex-1">
                          <div className="text-sm capitalize leading-tight">
                            {s.instruction.replace(/_/g, " ")}
                          </div>
...
function PlaceField({
  ...
  const isHomeMatch =
    homePlace &&
    (!q ||
      "home".startsWith(q.toLowerCase()) ||
      homePlace.name.toLowerCase().includes(q.toLowerCase()));
  const hits =
    !value && isHomeMatch && homePlace
          ...
          {hits.map((h) => (
            <li key={h.id}>
              <button
                ...
                className="w-full text-left px-2 py-1.5 hover:bg-panel-2 flex items-start gap-2"
              >
                {h.id === homePlace?.id && (
                  <Home className="size-4 text-orange-500 mt-0.5 shrink-0" />
                )}
                <div className="overflow-hidden w-full">
                  <div className="text-sm">{h.id === homePlace?.id ? "Home" : h.name}</div>
                  <div className="font-mono text-[10px] text-muted-foreground truncate">
                    {h.id === homePlace?.id ? h.name : h.displayName}
                  </div>
                </div>
              </button>
            </li>
