...
export default function RightPanel() {
  ...
  const {
    ...
    isFetching: isDeepDiveFetching,
    refetch: fetchDeepDive,
  } = useQuery({
  ...
  const loading =
    isGeocoding || isWikiLoading || isWeatherLoading || isNearbyLoading || isCountryLoading;
      ...
      <aside className="h-full bg-panel border-l border-line p-6 overflow-y-auto">
        ...
        >
          ...
          <p className="font-serif text-xl leading-snug text-muted-foreground">
            Nothing selected. The map is waiting. Click a country, a coast, a dot in the middle of
            nowhere — and a dossier will appear here.
          </p>
          <div className="mt-8 border-t border-line-soft pt-4 space-y-2 font-mono text-[10px] text-muted-foreground">
            <p>
              <span className="text-accent">tip</span> — clicks reverse-geocode through
              OpenStreetMap.
            </p>
            <p>
              <span className="text-accent">tip</span> — facts come from Wikipedia, Open-Meteo, REST
              Countries, USGS.
            </p>
            <p>
              <span className="text-accent">tip</span> — no Google. No API keys. Just open data.
            </p>
          </div>
        </motion.div>
      </aside>
        ...
        >
          ...
          <div className="px-5 pt-5 pb-4 border-b border-line">
            ...
            <div className="mt-3 flex items-center gap-4">
              ...
              >
                ...
                {homePlace?.id === p!.id ? "remove home" : "set as home"}
              </button>
          ...
          {loading && (
            <div className="px-5 py-6 flex items-center gap-2 font-mono text-[10px] text-muted-foreground">
              <Loader2 className="size-3 animate-spin" /> gathering everything we know about this
              place…
            </div>
          )}
              ...
              <div className="flex items-start gap-3">
                {wiki.thumbnail && (
                  <img
                    src={wiki.thumbnail}
                    alt=""
                    className="w-20 h-20 object-cover border border-line grayscale-[0.2]"
                  />
                )}
                <div className="flex-1">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-1 mt-2 font-mono text-[10px] text-accent hover:underline"
                  >
          ...
          <section className="px-5 py-4 border-b border-line bg-panel-2">
            ...
            {Object.entries(savedLists).map(([listName, items]) => {
              ...
              return (
                <div
                  key={listName}
                  className="flex items-center justify-between bg-panel border border-line px-3 py-2 mb-2 text-[11px] font-mono"
                >
                  <span className="text-accent">In {listName}</span>
                  <button
                    onClick={() => removeFromList(listName, p!.id)}
                  ...
                  <ul className="space-y-2">
                    {connections.map((c) => (
                      <li key={c.name}>
                        ...
                        >
                          <div className="text-sm text-sky-500 group-hover:underline leading-tight">
                            {c.name}
                          </div>
              ...
              <ul className="divide-y divide-line-soft">
                {nearby.slice(0, 10).map((n) => (
                  <li key={n.title}>
                    ...
                    >
                      <span className="font-mono text-[10px] text-muted-foreground tabular-nums w-12">
                        {Math.round(n.dist)}m
                      </span>
